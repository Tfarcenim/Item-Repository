package tfar.itemrepository.command;
...
public class RepositoryCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register(Commands.literal(ItemRepository.MODID)
    ...
    private static int clearAll(CommandContext<CommandSourceStack> context) {
        ItemRepository.instance.data.clearAll();
        return 1;
    }

    private static int clearID(CommandContext<CommandSourceStack> context) {
        int id = IntegerArgumentType.getInteger(context, "id");
        boolean success = ItemRepository.instance.data.clearId(id);
        if (!success) {
